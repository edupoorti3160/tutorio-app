...
export default function Classroom() {
    ...
    // CORRECCIÓN 1: Detectar roomid (minúscula) o roomId (mayúscula) para evitar el círculo infinito
    ...
    const { localStream, remoteStream, isConnected } = useWebRTC(
        (hasJoined && !isRoleLoading && userId && roomId) ? roomId : '', 
        userId || '', 
        isInitiator
    );
    ...
    // --- 1. ROL Y DATOS (LÓGICA BLINDADA) ---
    useEffect(() => {
        const fetchUserRoleAndBooking = async () => {
            if (!roomId) return; // Si no hay ID, no hacemos nada (evita errores)
            ...
            try {
                ...
                if (!user) {
                    ...
                    setUserId(user.id);
                    // Usamos limit(1) para evitar el error 406 en perfiles duplicados
                    ...
                    if (role === 'teacher') {
                        ...
                        // CORRECCIÓN 2: Si es una llamada instantánea, NO BUSCAMOS EN BOOKINGS
                        // Esto elimina el error 406 de raíz en la sala.
                        const isInstantCall = roomId.startsWith('instant-');
                        ...
                        if (!isInstantCall) {
                            const { data: bookingData } = await supabase.from('bookings')
                                ...
                                .limit(1); // Usamos limit(1) por seguridad
                            
                            if (bookingData && bookingData.length > 0) {
                                setBookingDetails(bookingData[0]);
                            }
                        } else {
                            console.log("Instant Call: Skipping bookings fetch to avoid 406 error.");
                        }
    ...
    const startRecognition = useCallback(() => {
        ...
        recognition.onend = () => { 
            if (isVoiceActiveRef.current) setTimeout(() => { if (isVoiceActiveRef.current) recognition.start(); }, 200);
            else setInterimSubtitle('');
        };
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (translatedText && translatedText.toLowerCase() !== originalText.toLowerCase()) {
                setMessages(prev => [...prev, { sender: "Traductor", text: translatedText, isTranslation: true }]);
            }
    ...
    const handleShareResource = async (resource: any) => {
        if (resource.file_type === 'image' || resource.file_url?.match(/\.(jpeg|jpg|gif|png|webp)$/i)) {
            ...
            await supabase.channel(`room-${roomId}`).send({
                ...
                payload: { x: 200, y: 150, url: resource.file_url }
            });
        } else {
        ...
        <div className="h-screen bg-slate-100 flex flex-col font-sans overflow-hidden">
            <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0 z-10 shadow-sm">
                ...
                <div className="flex gap-4 items-center">
                    <button onClick={toggleVoiceTranslator} className={`flex items-center gap-3 px-5 py-2 rounded-full border-2 transition-all shadow-sm ${isVoiceActive ? 'bg-red-50 border-red-500 text-red-600 ring-2 ring-red-100' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                        {isSpeaking && <Volume2 className="w-5 h-5 animate-bounce text-indigo-600"/>}
                        <span className="font-bold text-sm tracking-wide">{isVoiceActive ? t.translatingBtn : t.translateBtn}</span>
                    ...
                    <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-xl border-2 border-slate-200 shadow-sm">
                        ...
                        <div className="relative group">
                            <select value={sourceLang} onChange={(e) => setSourceLang(e.target.value)} className="appearance-none bg-transparent font-bold text-slate-700 outline-none cursor-pointer text-sm pr-6 hover:text-indigo-600">
                                {SUPPORTED_LANGUAGES.map(lang => (<option key={`src-${lang.code}`} value={lang.code}>{lang.flag} {lang.code.toUpperCase()}</option>))}
                            </select>
                        ...
                        <div className="relative group">
                            <select value={targetLang} onChange={(e) => setTargetLang(e.target.value)} className="appearance-none bg-transparent font-bold text-indigo-600 outline_none cursor-pointer text-sm pr-6 hover:text-indigo-800">
                                {SUPPORTED_LANGUAGES.map(lang => (<option key={`tgt-${lang.code}`} value={lang.code}>{lang.flag} {lang.code.toUpperCase()}</option>))}
                            </select>
            ...
            <div className="flex-1 flex overflow-hidden relative">
                <div className="flex-1 bg-slate-100 relative flex flex-col items-center justify-center p-4">
                    <div className="w-full h-full border border-slate-300 rounded-2xl overflow-hidden shadow-lg bg-white relative">
                        <Whiteboard roomId={roomId as string} role={(userRole || 'student') as 'teacher' | 'student'} />
                        {activeResource && (
                            <div className="absolute inset-0 pointer-events-none flex items-center justify-center z-0" style={{ transform: `translate(${resourcePosition.x}px, ${resourcePosition.y}px) scale(${resourceScale}) rotate(${resourceRotation}deg)`, transition: 'transform 0.1s linear', transformOrigin: 'center center' }}>
