...
export default function TeacherMessages() {
  ...
  // IDIOMA PREFERIDO DEL MAESTRO (Por defecto Español, pero cambiable)
  const [myLanguage, setMyLanguage] = useState('es') 
  const [showLangMenu, setShowLangMenu] = useState(false)
  
  const messagesEndRef = useRef<HTMLDivElement>(null)

  // 1. CARGAR USUARIO Y CONTACTOS (ESTUDIANTES)
  useEffect(() => {
  ...
  useEffect(() => {
          ...
          if (data) {
              setMessages(data.map((m: any) => {
                  ...
                  return {
                      ...
                      content: m.content,
                      // Mostrar traducción solo si es del otro y existe
      ...
      const channel = supabase.channel('chat_room_teacher')
  ...
  return (
    <div className="h-screen bg-slate-50 p-4 md:p-8 font-sans text-slate-900 flex flex-col">
      ...
      <div className="flex items-center gap-4 mb-6 shrink-0">
        ...
        <h1 className="text-2xl font-bold text-slate-900">Mensajes (Maestro)</h1>
      </div>
        ...
        <div className={`w-full md:w-1/3 border-r border-slate-100 bg-slate-50/50 flex flex-col ${activeChatId ? 'hidden md:flex' : 'flex'}`}>
          <div className="p-4 border-b border-slate-100">
            <div className="relative">
                ...
                <input type="text" placeholder="Buscar alumnos..." className="w-full pl-9 pr-4 py-2 bg-white border border-slate-200 rounded-lg text-sm outline-none focus:border-indigo-500" />
            </div>
          </div>
